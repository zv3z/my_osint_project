...
def _whois(target, ttype):
    try:
        r = requests.get("https://www.whoisxmlapi.com/whoisserver/WhoisService",
                         params={"apiKey": CONF["WHOIS_KEY"], "domainName": target, "outputFormat": "JSON"},
                         timeout=12)
...
def _leakcheck(target, ttype):
    try:
        r = requests.get(f"https://leakcheck.io/api?key={CONF['LEAKCHECK_KEY']}&type=email&check={target}", timeout=12)
        return r.json()
    except Exception as e:
        return {"error": str(e)}
...
def _breach(target, ttype):
    try:
        r = requests.get(f"https://breachdirectory.p.rapidapi.com/?func=auto&term={target}",
                         headers={"X-RapidAPI-Key": CONF["BREACH_KEY"],
...
def _intelx(target, ttype):
    try:
        r = requests.get(f"https://2.intelx.io/intelligent/search",
                         params={"k": CONF["INTELX_KEY"], "q": target, "limit": 5}, timeout=12)
        return r.json()
    except Exception as e:
        return {"error": str(e)}
...
def _hunter(target, ttype):
    try:
        ...
        r = requests.get(f"https://api.hunter.io/v2/domain-search",
                         params={"domain": domain, "api_key": CONF["HUNTER_KEY"]}, timeout=12)
        d = r.json().get("data", {})
